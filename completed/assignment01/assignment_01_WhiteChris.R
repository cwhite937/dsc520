...
students <- c(student01, student02, student03)
students_combined_matrix <- matrix(students, byrow = TRUE, nrow = 3)



## Add a new student row with `rbind()`
student04 <- c(95.2, 94.1)
grades_matrix <- rbind(students_combined_matrix, student04)


## Add a new assignment column with `cbind()`
assignment04 <- c(92.1, 84.3, 75.1, 97.8)
more_grades_matrix <- cbind(grades_matrix, assignment04)
...
rownames(more_grades_matrix) <- students
colnames(more_grades_matrix) <- assignments

## Total points for each assignment using `colSums()`
total_points_assignment <- colSums(more_grades_matrix)
total_points_assignment


## Total points for each student using `rowSums()`
total_points_student <- rowSums(more_grades_matrix)
